    ...
    def __init__(self, datasets, mode='train', transform=None, return_size=True):
        ...
        for (i, dataset) in enumerate(datasets):
            ...
            if mode == 'train':
                ...
                imgset_path = data_dir + '/train_depth.txt'

            else:
                data_dir = './dataset/test/{}'.format(dataset)
                imgset_path = data_dir + '/test_depth.txt'

            imgset_file = open(imgset_path)

            for line in imgset_file:
                data = {}
                img_path = data_dir + line.strip("\n").split(" ")[0]#img_path = line.strip("\n").split(" ")[0]
                gt_path = data_dir + line.strip("\n").split(" ")[1]#gt_path = line.strip("\n").split(" ")[1]
                data['img_path'] = img_path#data['img_path'] = data_dir + img_path
                data['gt_path'] = gt_path#data['gt_path'] = data_dir + gt_path
                if dataset == 'DUTS':
                    data['split'] = dataset#将数据集的名称存储到data字典中，用于跟踪当前处理的数据子集属于哪个划分（比如训练集、验证集还是测试集）
                    # DUTS Depth
                    # data['depth_path'] = data_dir + line.strip("\n").split(" ")[-1]
                else:
                    data['flow_path'] = data_dir + line.strip("\n").split(" ")[2]
                    data['depth_path'] = data_dir + line.strip("\n").split(" ")[3]
                    data['split'] = data['img_path'].split('/')[-2]#data['split'] = img_path.split('/')[-3]#从/blackswan/Imgs/blackswan_00000.jpg中得到blackswan。  ["","blackswan","Imgs","blackswan_00000.jpg"]
                data['dataset'] = dataset
                self.datas_id.append(data)
        self.transform = transform

    def __getitem__(self, item):
        #self.datas_id[item]['img_path']从datas_id中根据item取出相应的字典，并从中获取‘img_path’键对应的值
        ...
        if self.datas_id[item]['dataset'] == 'DUTS':
            pass
            # DUTS Depth
        ...
        if self.datas_id[item]['dataset'] == 'DUTS':
            sample['flow'] = torch.zeros((3, 384, 384))
            # DUTS Depth
            sample['depth'] = torch.zeros((3, 384, 384))
